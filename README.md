...
| **선택적 LLM 파서** | `USE_LLM=1`일 때 자연어 구조화에만 사용하고 실패 시 규칙 파서로 폴백 |
...
선택적으로 LLM 파서를 사용하려면 `USE_LLM=1`과 `OPENAI_API_KEY`를 설정합니다. 세부 모델 라우팅은 [`app/README.md`](app/README.md)에 설명되어 있습니다.
