...
![Tests](https://img.shields.io/badge/tests-180%20passed%20%C2%B7%20cov%2099%25-brightgreen)
...
현재 **전원마을 분양정보(전국 167곳)를 실시간 조회**하며, 서비스키가 없으면 샘플 데이터로 같은 흐름을 실행합니다. 농촌마을현황·논가뭄지도는 아직 실시간 연동 전이라 인구·빈집·가뭄 패널이 &lsquo;확인 불가&rsquo;로 표시됩니다 — 화면과 API는 이 상태를 숨기지 않습니다. 실제 분양·계약 판단은 공식 기관 확인이 필요합니다.
...
| **신뢰도 A~D** | 법정동코드 직접 매칭 여부로 결합 수준 표시 (live에서는 마을현황 미연동으로 전부 `C`) |
...
| 농촌마을현황 | `15104291` | 인구·빈집·지역 자원 | 미연동 (2.8만 건 조인 설계 필요) |
...
- `pytest`: **180 passed, 2 skipped** · 커버리지 **99%**
...
- [ ] **농촌마을현황(2.8만 건) 조인** — 미완. 인구·빈집이 &lsquo;확인 불가&rsquo;이고 신뢰도 등급이 전부 `C`로 고정되는 원인
